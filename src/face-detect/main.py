    ...
    for idx in range(current_idx, lenvid, opt.total_processes):
        ...
        if not os.path.isfile(out_path+'detections.pkl'):
            ...
            for tracks in tracked_out:
                ...
                fix_and_crop_bbox_size(out_dir=out_path, bboxes=tracklet_bboxes, frame_ids=tracklet_frames, paths=paths, scale=opt.scale, im_w=width, im_h=height, trackid=i, workers=opt.workers)

            # TODO: Add the cropping part over here?
            with open(out_path+'detections.pkl', 'wb') as handle:
               pickle.dump((out,tracked_out,paths), handle, protocol=pickle.HIGHEST_PROTOCOL)    
