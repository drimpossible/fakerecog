...
def crop_im(outdir, burst_path, framepath, trackid, bbox):
    framepath = framepath.split('/')[-1]
    img = Image.open(burst_path+"/"+framepath)
    crop = img.crop(bbox.numpy()) 
    framepath = '/track'+str(trackid)+"_"+framepath
    crop.save(outdir+framepath)

def diff_crop_im(outdir, burst_path, orig_burst_path, framepath, trackid, bbox):
    framepath = framepath.split('/')[-1]
    img1 = Image.open(orig_burst_path+"/"+framepath).crop(bbox.numpy())
    img2 = Image.open(burst_path+"/"+framepath).crop(bbox.numpy())
